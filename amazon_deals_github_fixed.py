#!/usr/bin/env python3
"""
Amazon Deal Bot for GitHub Actions

This script finds Amazon products with 50% or more discount,
adds affiliate links, and uploads the data to Pastebin
...
class PastebinUploader:
    ...
    def __init__(self, api_dev_key=None):
        ...
        self.paste_key = None  # Store the paste key for updates
        
    def upload_deals_as_csv(self, deals, title="Amazon Deals 50% Off or More", update_existing=False):
        """
        ...
        Args:
            ...
            update_existing: Whether to update an existing paste instead of creating a new one
        ...
        if self.api_dev_key:
            ...
            if update_existing and self.paste_key:
                return self._update_via_api(csv_content, title)
            else:
                return self._upload_via_api(csv_content, title)
        else:
            # Use anonymous paste if no API key
            return self._upload_anonymous(csv_content, title)
    
    def _upload_via_api(self, content, title):
        """
        Upload content to Pastebin using the API.
        ...
        try:
            ...
            if response.status_code == 200 and not response.text.startswith('Bad API request'):
                ...
                logger.info(f"Successfully uploaded to Pastebin: {pastebin_url}")
                return pastebin_url
            else:
                logger.error(f"Failed to upload to Pastebin API: {response.text}")
                return None
        
        except Exception as e:
            logger.error(f"Error uploading to Pastebin API: {e}")
            return None
    
    def _update_via_api(self, content, title):
        """
        Update an existing Pastebin paste using the API.
        ...
        try:
            ...
            if response.status_code == 200 and not response.text.startswith('Bad API request'):
                ...
                # Extract paste key from URL
                self.paste_key = pastebin_url.split('/')[-1]
                logger.info(f"Successfully updated Pastebin: {pastebin_url}")
                return pastebin_url
            else:
                logger.error(f"Failed to update Pastebin API: {response.text}")
                # If update fails, try creating a new paste
                logger.info("Attempting to create a new paste instead")
                return self._upload_via_api(content, title)
        
        except Exception as e:
            logger.error(f"Error updating Pastebin API: {e}")
            return None
...
def save_paste_key(paste_key, filename="paste_key.txt"):
    """Save the Pastebin paste key to a file for future updates."""
    try:
        with open(filename, 'w') as f:
            f.write(paste_key)
        logger.info(f"Saved paste key to {filename}")
    except Exception as e:
        logger.error(f"Error saving paste key: {e}")


def load_paste_key(filename="paste_key.txt"):
    """Load the Pastebin paste key from a file."""
    try:
        if os.path.exists(filename):
            with open(filename, 'r') as f:
                paste_key = f.read().strip()
            logger.info(f"Loaded paste key from {filename}")
            return paste_key
        else:
            logger.info(f"No paste key file found at {filename}")
            return None
    except Exception as e:
        logger.error(f"Error loading paste key: {e}")
        return None
...
def main():
    ...
    # Initialize the Pastebin uploader
    pastebin_uploader = PastebinUploader(api_dev_key=api_key)
    
    # Load existing paste key if available
    paste_key = load_paste_key()
    if paste_key:
        pastebin_uploader.paste_key = paste_key
    ...
    if not args.simulate and api_key:
        ...
        if pastebin_url:
            ...
            # Save the paste key for future updates
            if pastebin_uploader.paste_key:
                save_paste_key(pastebin_uploader.paste_key)
        else:
